...
async def debug_sources():
    """Diagnostic: test yt-dlp and transcript API for a known video."""
    import asyncio
    ...
    try:
        yt = YouTubeExtractor()
        info = await asyncio.to_thread(yt.extract_video_detail,
            "https://www.youtube.com/watch?v=jNQXAC9IVRw")
        results["ytdlp"] = "OK" if info else "empty"
    except Exception as e:
        results["ytdlp"] = str(e)[:200]

    # transcript-api (subtitles)
    try:
        subs = SubtitleService("")
        sub_result = await subs.fetch_subtitles("jNQXAC9IVRw", ["en"], "auto")
        results["transcript_api"] = sub_result["status"] if sub_result else "empty"
    except Exception as e:
        results["transcript_api"] = str(e)[:200]
