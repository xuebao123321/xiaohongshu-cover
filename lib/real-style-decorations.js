...
function slotToBox(slot) {
  const margin = 60;
  switch (slot) {
    case 'topLeft':      return { x: margin, y: 80, w: 160, h: 120 };
    case 'topRight':     return { x: CW - margin - 160, y: 80, w: 160, h: 120 };
    case 'bottomLeft':   return { x: margin, y: CH - 280, w: 160, h: 140 };
    case 'bottomRight':  return { x: CW - margin - 160, y: CH - 280, w: 160, h: 140 };
    case 'aboveTitle':   return { x: CW * 0.15, y: 220, w: CW * 0.70, h: 80 };
    case 'belowTitle':   return { x: CW * 0.20, y: CH - 380, w: CW * 0.60, h: 80 };
    case 'aroundKeyword':return { x: CW * 0.25, y: 300, w: CW * 0.50, h: 70 };
    default:             return { x: CW * 0.30, y: CH * 0.60, w: 400, h: 100 };
  }
}
...
function buildHandnoteDecorations(slots, rng, palette, targetCount) {
  ...
  // 必选：胶带
  const tapeSlot = pickRandom(slots.filter(s => s.startsWith('top')), rng) || slots[0] || 'topLeft';
  decorations.push({
    id: decoId('tape'),
    type: 'tape',
    slot: tapeSlot,
    ...
    drawInstruction: {
      type: 'rect',
      params: {
        x: tapeSlot.includes('Left') ? 50 : CW - 280,
        y: 50 + rng() * 30,
        w: 200 + rng() * 60,
        h: 22 + rng() * 10,
        color: 'rgba(230,210,170,0.50)',
        rotation: (rng() - 0.5) * 0.15,
      },
    },
  });
  ...
  for (const oslot of optSlots) {
    ...
    if (choice < 0.33) {
      ...
      decorations.push({
        ...
        drawInstruction: {
          ...
          params: { x: obox.x + obox.w / 2, y: obox.y + obox.h / 2,
                    emoji: pickRandom(emojis, rng), fontSize: 42 + rng() * 20 },
        },
      });
    } else {
      // 小圆点
      decorations.push({
        ...
        drawInstruction: {
          type: 'dot',
          params: { cx: obox.x + obox.w / 2, cy: obox.y + obox.h / 2, r: 4 + rng() * 6,
