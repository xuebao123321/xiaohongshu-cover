...
function slotToBox(slot) {
  var margin = 40;
  switch (slot) {
    case 'topLeft':      return { x: margin, y: 60, w: 260, h: 200 };
    case 'topRight':     return { x: CW - margin - 260, y: 60, w: 260, h: 200 };
    case 'bottomLeft':   return { x: margin, y: CH - 340, w: 260, h: 200 };
    case 'bottomRight':  return { x: CW - margin - 260, y: CH - 340, w: 260, h: 200 };
    case 'aboveTitle':   return { x: CW * 0.10, y: 180, w: CW * 0.80, h: 100 };
    case 'belowTitle':   return { x: CW * 0.15, y: CH - 420, w: CW * 0.70, h: 100 };
    case 'aroundKeyword':return { x: CW * 0.20, y: 260, w: CW * 0.60, h: 90 };
    default:             return { x: CW * 0.25, y: CH * 0.60, w: 500, h: 140 };
  }
}
...
function buildHandnoteDecorations(slots, rng, palette, targetCount) {
  ...
  decorations.push({
    ...
    drawInstruction: {
      type: 'rect',
      params: {
        x: tapeSlot.includes('Left') ? 40 : CW - 280,
        y: 45 + rng() * 30,
        w: 190 + rng() * 90,   // ══════ P3: 胶带 180-300px ══════
        h: 26 + rng() * 12,
