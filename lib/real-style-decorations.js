      ...
      decorations.push({
        ...
        drawInstruction: {
          ...
          params: {
            ...
            r: 50 + rng() * 40,
            fill: 'transparent',
            stroke: palette.accent || '#B0823B',
            lineWidth: 3,
            label: '印',
            labelColor: palette.accent || '#B0823B',
            fontSize: 24,
          },
        },
      });
...
function buildMinimalDecorations(slots, rng, palette) {
  var decorations = [];
  // minimal: 0-1个装饰，只用粗横线（大而明确）
  if (slots.length === 0 || rng() > 0.5) return decorations;

  var oslot = slots[0];
  var obox = slotToBox(oslot);
  // 粗横线托底 — 大尺寸可视
  decorations.push({
    id: decoId('boldline'),
    type: 'line',
    ...
    drawInstruction: {
      type: 'line',
      params: {
        x1: obox.x + obox.w * 0.3, y1: obox.y + obox.h / 2,
        x2: obox.x + obox.w * 0.7, y2: obox.y + obox.h / 2,
        stroke: palette.accent || palette.text || '#111111',
        lineWidth: 5,
      },
    },
  });
    ...
    try {
      switch (inst.type) {
        ...
        case 'path':
          ...
          break;
        case 'sticker':
          // 对标小红书贴纸效果: Canvas手绘+白边+投影+旋转
          if (params.stickerId && typeof window.renderAsSticker === 'function' && window._drawStickerFns) {
            try {
              const SW = R_CANVAS_W, SH = R_CANVAS_H;
              const stickerSize = params.w || Math.round(SW * (0.08 + Math.random() * 0.05));
              const cx = params.x || Math.round(SW * (0.12 + Math.random() * 0.16));
              const cy = params.y || Math.round(SH * (0.72 + Math.random() * 0.20));
              const rotation = (params.rotation != null) ? params.rotation : ((Math.random() - 0.5) * 10);

              // 优先: 用户上传的自定义贴纸(本地+云端)
              const customStickers = (typeof window.getCustomStickers === 'function') ? window.getCustomStickers() : [];
              if (customStickers.length > 0 && Math.random() < 0.25) {
                const cs = customStickers[Math.floor(Math.random() * customStickers.length)];
                const img = new Image();
                img.src = cs.dataUrl;
                const tryDraw = () => {
                  if (img.complete && img.naturalWidth > 0) {
                    ...
                    const pad = stickerSize * 0.08; const ts = stickerSize + pad * 2;
                    ctx.shadowColor = 'rgba(0,0,0,0.14)'; ctx.shadowBlur = stickerSize * 0.08;
                    ctx.shadowOffsetX = stickerSize * 0.02; ctx.shadowOffsetY = stickerSize * 0.03;
                    ctx.fillStyle = '#FFFFFF'; roundRectPath(ctx, -ts/2, -ts/2, ts, ts, stickerSize * 0.1); ctx.fill();
                ...
                if (tryDraw()) return; // 自定义贴纸成功,跳过Canvas
                // 图片未就绪,等onload再试(但不阻塞,继续Canvas兜底)
                if (!img.complete) img.onload = () => { /* 下次渲染可用 */ };
              }

              // Canvas 手绘贴纸(无网络请求,无404)
              const fallbackName = (window.getStickerFallbackRef && window.getStickerFallbackRef(params.stickerId)) || '';
              let drawFn = window._drawStickerFns[fallbackName];
              if (!drawFn) {
                for (const key of Object.keys(window._drawStickerFns)) {
                  if (key.toLowerCase().includes(params.stickerId.toLowerCase().replace(/[^a-z]/g, '').slice(0, 8))) {
                    drawFn = window._drawStickerFns[key]; break;
                  }
                }
              }
              if (!drawFn) {
                const pool = [window._drawStickerFns.drawCuteCatFaceSticker,
                              window._drawStickerFns.drawHeart3DSticker,
                              window._drawStickerFns.drawDoodleStarSticker];
                drawFn = pool[Math.floor(Math.random() * pool.length)];
              }
              if (drawFn) {
                window.renderAsSticker(ctx, drawFn, cx, cy, stickerSize, rotation, '#E6213D');
              }
            } catch (e) {
              // 贴纸渲染失败静默跳过
            }
