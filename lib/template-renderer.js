...
function injectTemplateStickers(ctx, styleFamily, options) {
  ...
  var fns = (window._drawStickerFns) || {};
  var keys = Object.keys(fns);
  if (keys.length === 0) return; // 贴纸函数未就绪
  ...
  for (var i = 0; i < count; i++) {
    // 从所有可用贴纸函数中随机选
    var idx = Math.floor(rng(seed + i * 7919) * keys.length);
    var fnName = keys[idx];
    var drawFn = fns[fnName];
    if (typeof drawFn !== 'function') continue;

    var size = Math.round(TPL_CW * (0.07 + rng(seed + i * 313) * 0.06));
    var cx = Math.round(TPL_CW * (0.08 + rng(seed + i * 617) * 0.70));
    var cy = Math.round(TPL_CH * (0.55 + rng(seed + i * 419) * 0.35));
    var rot = (rng(seed + i * 523) - 0.5) * 8;

    ctx.save();
    try {
      // 手动画白边+投影(不依赖外部renderAsSticker,确保可见)
      ctx.translate(cx, cy);
