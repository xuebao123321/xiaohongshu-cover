...
function injectTemplateStickers(ctx, styleFamily, options) {
  ...
  for (var i = 0; i < count; i++) {
    // 优先PNG云端贴纸, Canvas兜底
    var size = Math.round(TPL_CW * (0.12 + rng(seed + i * 313) * 0.06));
    var cx = Math.round(TPL_CW * (0.08 + rng(seed + i * 617) * 0.72));
    var cy = Math.round(TPL_CH * (0.45 + rng(seed + i * 419) * 0.45));
    ...
    try {
      ...
      ctx.shadowBlur = size * 0.08; ctx.shadowOffsetX = 2; ctx.shadowOffsetY = 4;
