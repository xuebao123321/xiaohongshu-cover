...
function injectTemplateStickers(ctx, styleFamily, options) {
  ...
  for (var i = 0; i < count; i++) {
    var size = Math.round(TPL_CW * (0.08 + rng(seed + i * 313) * 0.06));
    var cx = Math.round(TPL_CW * (0.08 + rng(seed + i * 617) * 0.72));
    var cy = Math.round(TPL_CH * (0.52 + rng(seed + i * 419) * 0.38));
    var rot = (rng(seed + i * 523) - 0.5) * 8;

    // 80%概率用自定义PNG,20%用Canvas兜底
    ...
    try {
      ...
      if (useCustom) {
        // PNG贴纸
        var cs = customStickers[Math.floor(rng(seed + i * 877) * customStickers.length)];
      ...
      if (!useCustom) {
        // Canvas兜底: 选一个好看的
        var idx = Math.floor(rng(seed + i * 7919) * cuteKeys.length);
        var drawFn = cuteFns[cuteKeys[idx]];
        if (typeof drawFn === 'function') drawFn(ctx, 0, 0, size, '#E6213D');
      }
    } catch(e) { /* skip */ }
    ctx.restore();
  }
