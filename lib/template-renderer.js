...
function renderTemplate(ctx, templateId, content, options) {
  ...
  try {
    ...
    // 2. 模板装饰(模板定义的装饰随机1-3个,文字下层)
    drawRandomDecorations(ctx, tpl, options);

    // 3. 贴纸补充(可爱元素,文字下层)
    injectTemplateStickers(ctx, tpl.styleFamily, options);

    // 4. 标题 — 模板定义的 titleBox(对标参考图)
...
/** 简洁装饰框(对标参考图的画面结构,不丑) */
function drawDecoFrame(ctx, tpl) {
  var style = tpl.styleFamily || 'handnote';
  var margin = 40, r = 24, w = TPL_CW - margin*2, h = TPL_CH - margin*2;
  if (style === 'minimal') return;
  var lw = (style === 'comic' || style === 'collage') ? 4 : 2;
  var alpha = (style === 'comic') ? 0.18 : 0.10;
  var color = tpl.titleFont ? tpl.titleFont.color || '#333' : '#333';
  ctx.save();
  ctx.strokeStyle = color; ctx.globalAlpha = alpha; ctx.lineWidth = lw;
  if (style === 'collage') {
    ctx.fillStyle = 'rgba(255,255,255,0.30)';
    ctx.beginPath(); ctx.roundRect(margin, margin, w, h, r); ctx.fill(); ctx.stroke();
  } else {
    ctx.beginPath(); ctx.roundRect(margin, margin, w, h, r); ctx.stroke();
  }
  if (style === 'handnote') {
    ctx.globalAlpha = 0.12; ctx.fillStyle = color;
    ctx.fillRect(TPL_CW*0.35, margin-2, TPL_CW*0.30, lw+4);
  }
  ctx.restore();
}

/** 从模板装饰池 + 同族跨模板池中随机抽1-3个,增加多样性 */
function drawRandomDecorations(ctx, tpl, options) {
  var ownDecos = tpl.decorations || [];
  var seed = (options && options.seed) || 42;
  var rng = function(s) { var x = Math.sin(s * 9999 + 137) * 10000; return x - Math.floor(x); };

  // 同族其他模板的装饰作为补充池(增加多样性)
  var familyPool = [];
  var lib = window.TEMPLATE_LIBRARY || {};
  var tplIds = Object.keys(lib).filter(function(id) {
    return id.indexOf(tpl.styleFamily) === 0 && id !== tpl.templateId;
  });
  // 随机取2个兄弟模板
  for (var k = 0; k < Math.min(2, tplIds.length); k++) {
    var idx = Math.floor(rng(seed + k * 419) * tplIds.length);
    var sibling = lib[tplIds[idx]];
    if (sibling && sibling.decorations) {
      familyPool = familyPool.concat(sibling.decorations);
    }
  }

  // 合并: 自己的装饰 + 同族兄弟的装饰
  var allDecos = ownDecos.concat(familyPool);
  if (allDecos.length === 0) return;

  // Fisher-Yates shuffle then take 1-3
  var shuffled = allDecos.slice();
  for (var i = shuffled.length - 1; i > 0; i--) {
    var j = Math.floor(rng(seed + i * 313) * (i + 1));
    var tmp = shuffled[i]; shuffled[i] = shuffled[j]; shuffled[j] = tmp;
  }

  var count = Math.min(shuffled.length, 1 + Math.floor(rng(seed + 617) * 3));
  for (var i = 0; i < count; i++) {
    var d = shuffled[i];
    var b = d.box;
    // 随机微调位置(±5%), 避免总是同一位置
    var bx = b.x + (rng(seed + i * 731) - 0.5) * b.w * 0.1;
    var by = b.y + (rng(seed + i * 829) - 0.5) * b.h * 0.1;
    ctx.save();
    try {
      if (d.rotation) { ctx.translate(bx + b.w / 2, by + b.h / 2); ctx.rotate(d.rotation * Math.PI / 180); ctx.translate(-(bx + b.w / 2), -(by + b.h / 2)); }
      switch (d.type) {
        case 'tape':         drawTapeDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'handCircle':   drawHandCircleDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'highlighter':  drawHighlighterDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'stamp':        drawStampDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'tornNote':     drawTornNoteDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'arrow':        drawArrowDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'colorBlock':   drawColorBlockDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'badge':        drawBadgeDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'doodle':       drawDoodleDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'emoji':        drawEmojiDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'speechBubble': drawBubbleDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'sfx':          drawSfxDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'starBurst':    drawStarBurstDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'divider':      drawDividerDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'thickLine':    drawThickLineDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'polaroid':     drawPolaroidDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
        case 'markerGhost':  drawMarkerGhostDeco(ctx, {x:bx, y:by, w:b.w, h:b.h}, d); break;
      }
    } catch (e) { /* deco fail → skip */ }
    ctx.restore();
  }
...
/** 自动注入贴纸到模板渲染(自定义PNG优先 → Canvas兜底) */
function injectTemplateStickers(ctx, styleFamily, options) {
  if (styleFamily === 'minimal') return;
  var seed = (options && options.seed) || 42;
  var rng = function(s) { var x = Math.sin(s * 9999 + 137) * 10000; return x - Math.floor(x); };
  var countRanges = { handnote: [1,2], collage: [2,3], comic: [2,3], newspaper: [1,2], minimal: [0,0] };
  var range = countRanges[styleFamily] || [1, 2];
  var count = range[0] + Math.floor(rng(seed + 1) * (range[1] - range[0] + 1));

  var customStickers = (typeof window.getCustomStickers === 'function') ? window.getCustomStickers() : [];
  var cloudImages = window._cloudStickerImages || {};
  var cloudList = window._cloudStickers || [];
  var hasCloud = cloudList.length > 0;
  var cuteFns = (window._drawStickerFns) || {};
  var cuteKeys = Object.keys(cuteFns);

  var usedStickers = {}; // 同张图不重复

    var titleBox = tpl.titleBox || { x: TPL_CW*0.10, y: TPL_CH*0.25, w: TPL_CW*0.80, h: TPL_CH*0.50 };
  var avoidText = (titleBox.w * titleBox.h) / (TPL_CW * TPL_CH) < 0.55;
  var safeZones = [
    { cx:0.15,cy:0.14 },{ cx:0.85,cy:0.14 },{ cx:0.15,cy:0.84 },{ cx:0.85,cy:0.84 },
    { cx:0.12,cy:0.68 },{ cx:0.88,cy:0.68 },
  ];

  for (var i = 0; i < count; i++) {
    var size = Math.round(TPL_CW * (0.11 + rng(seed + i * 313) * 0.06));
    var cx, cy;
    if (avoidText) {
      var z = safeZones[Math.floor(rng(seed + i * 731) * safeZones.length)];
      cx = Math.round(TPL_CW * (z.cx + (rng(seed + i * 617) - 0.5) * 0.10));
      cy = Math.round(TPL_CH * (z.cy + (rng(seed + i * 419) - 0.5) * 0.08));
    } else {
      cx = Math.round(TPL_CW * (0.08 + rng(seed + i * 617) * 0.72));
      cy = Math.round(TPL_CH * (0.45 + rng(seed + i * 419) * 0.42));
    }
    var rot = (rng(seed + i * 523) - 0.5) * 10;
    ctx.save();
    try {
      ctx.translate(cx, cy);
      ...
      ctx.shadowBlur = size * 0.06; ctx.shadowOffsetX = 2; ctx.shadowOffsetY = 3;

      // 优先PNG
      var drawn = false;
      if (hasCloud && rng(seed + i * 691) < 0.85) {
        var ci = Math.floor(rng(seed + i * 877) * cloudList.length);
        if (!usedStickers['cloud_' + ci]) {
          usedStickers['cloud_' + ci] = true;
          var cs = cloudList[ci];
          var img = cloudImages[cs.file] || null;
          if (!img) { img = new Image(); img.crossOrigin = 'anonymous'; img.src = cs.dataUrl; }
      ...
      // Canvas兜底
      if (!drawn && cuteKeys.length > 0) {
        var idx, tries = 0;
        do { idx = Math.floor(rng(seed + i * 7919 + tries * 137) * cuteKeys.length); tries++; }
        while (usedStickers[idx] && tries < 20);
        usedStickers[idx] = true;
        var drawFn = cuteFns[cuteKeys[idx]];
        if (typeof drawFn === 'function') drawFn(ctx, -size/2, -size/2, size, '#E6213D');
      }
      ctx.shadowColor = 'transparent';
    } catch(e) { /* skip */ }
    ctx.restore();
