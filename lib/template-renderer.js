...
function injectTemplateStickers(ctx, styleFamily, options) {
  if (styleFamily === 'minimal') return; // 极简风格不加贴纸
  if (!window.renderAsSticker || !window._drawStickerFns) return;
  ...
  var count = range[0] + Math.floor(rng(seed) * (range[1] - range[0] + 1));

  // 贴纸池(按风格族)
  var pools = {
    handnote: ['drawCuteCatFaceSticker','drawCuteDogFaceSticker','drawCoffeeCupSticker','drawFlowerSticker','drawCloverSticker','drawDoodleStarSticker','drawScribbleCloudSticker'],
    collage: ['drawFullBodyCatSticker','drawFullBodyDogSticker','drawBoyCharacterSticker','drawGirlCharacterSticker','drawHeart3DSticker','drawBalloon3DSticker','drawFlowerSticker'],
    comic: ['drawSparkleEyesSticker','drawAngryFaceSticker','drawShockedFaceSticker','drawLoveEyesSticker','drawFire3DSticker','drawStar3DSticker','drawFullBodyCatSticker','drawBoyCharacterSticker'],
    newspaper: ['drawCrown3DSticker','drawTrophy3DSticker','drawDiamond3DSticker','drawCoffeeCupSticker','drawRealisticStampSticker'],
  };
  var pool = pools[styleFamily] || pools.handnote;

  for (var i = 0; i < count; i++) {
    var idx = Math.floor(rng(seed + i * 7919) * pool.length);
    var fnName = pool[idx];
    var drawFn = window._drawStickerFns[fnName];
    if (!drawFn) continue;

    var size = TPL_CW * (0.08 + rng(seed + i * 313) * 0.05); // 8-13%画布宽
    var cx = TPL_CW * (0.10 + rng(seed + i * 617) * 0.60);
    var cy = TPL_CH * (0.60 + rng(seed + i * 419) * 0.32);
    var rot = (rng(seed + i * 523) - 0.5) * 10;

    try { window.renderAsSticker(ctx, drawFn, cx, cy, size, rot, '#E6213D'); } catch(e) {}
  }
}
