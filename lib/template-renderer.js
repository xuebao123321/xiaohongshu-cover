...
function injectTemplateStickers(ctx, styleFamily, options) {
  ...
  var usedStickers = {}; // 同张图不重复
    ...
    var customStickers = (typeof window.getCustomStickers === 'function') ? window.getCustomStickers() : [];
  var cloudList = window._cloudStickerList || [];
  var cloudImgs = window._cloudStickerImages || {};
  var cuteFns = (window._drawStickerFns) || {};
  var cuteKeys = Object.keys(cuteFns);
  var usedStickers = {};
  ...
  for (var i = 0; i < count; i++) {
    ...
    // 优先: base64云端PNG贴纸(透明底+投影)
    if (cloudList.length > 0 && rng(seed + i * 691) < 0.85) {
      var ci = Math.floor(rng(seed + i * 877) * cloudList.length);
      if (!usedStickers['cloud_' + ci]) {
        usedStickers['cloud_' + ci] = true;
        var img = cloudImgs[cloudList[ci].file];
        if (img && img.complete && img.naturalWidth > 0) {
          ctx.save(); ctx.translate(cx, cy); if (rot) ctx.rotate(rot * Math.PI / 180);
          ctx.shadowColor = 'rgba(0,0,0,0.18)'; ctx.shadowBlur = size * 0.06; ctx.shadowOffsetX = 2; ctx.shadowOffsetY = 3;
    ...
    if (!drawn && cuteKeys.length > 0) {
      ...
      if (typeof drawFn === 'function') {
        ctx.save(); ctx.translate(cx, cy); if (rot) ctx.rotate(rot * Math.PI / 180);
        ctx.shadowColor = 'rgba(0,0,0,0.18)'; ctx.shadowBlur = size * 0.06; ctx.shadowOffsetX = 2; ctx.shadowOffsetY = 3;
        drawFn(ctx, -size/2, -size/2, size, '#E6213D');
        ctx.shadowColor = 'transparent'; ctx.restore();
      }
    }
  }
