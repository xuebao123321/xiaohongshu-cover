...
function loadTemplate(templateId) {
  var cache = window._templateCache || (window._templateCache = {});
  ...
  cache[templateId] = JSON.parse(JSON.stringify(templates[templateId]));
...
function drawTemplateBackground(ctx, tpl) {
  ...
  // 聚焦圆形
  if (tpl.focusShape && tpl.focusShape.type === 'circle') {
...
function fitTextIntoBox(ctx, text, box, fontConf, family) {
  ...
  var fMin = (fontConf && fontConf.min) || 80;
  ...
  for (var iter = 0; iter < 20; iter++) {
    ...
    for (var ci = 0; ci < chars.length; ci++) {
      textW += ctx.measureText(chars[ci]).width;
      if (ci < chars.length - 1) textW += fontSize * fSpacing;
    }
    textH = fontSize * 1.10;
    if (textW <= box.w && textH <= box.h) break;
    fontSize = Math.floor(fontSize * 0.92);
  }
  fontSize = Math.max(fMin, Math.min(fMax, fontSize));

  // 居中绘制
  var x = box.x + (box.w - textW) / 2;
  var y = box.y + (box.h - textH) / 2;
  ctx.font = 'bold ' + fontSize + 'px ' + fFamily;
  ...
  for (var i = 0; i < chars.length; i++) {
    ...
    cx += ctx.measureText(chars[i]).width + fontSize * fSpacing;
  }
}
