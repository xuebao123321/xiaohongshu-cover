...
function renderTemplate(ctx, templateId, content, options) {
  ...
  try {
    ...
    // 1a. 装饰框: 简洁圆角边框(对标参考图的画面结构)
    drawDecoFrame(ctx, tpl);

    // 2. 🆕 贴纸层: 对标小红书风格的可爱贴纸(猫/狗/男孩/女孩/花/表情): 对标小红书风格的可爱贴纸(猫/狗/男孩/女孩/花/表情)
    injectTemplateStickers(ctx, tpl.styleFamily, options);

    // 3. 标题 — 全画布居中, 覆盖贴纸
    var fullBox = { x: TPL_CW * 0.06, y: TPL_CH * 0.18, w: TPL_CW * 0.88, h: TPL_CH * 0.64 };
    fitTextIntoBox(ctx, content.mainTitle || '', fullBox, tpl.titleFont, tpl.styleFamily);
...
function injectTemplateStickers(ctx, styleFamily, options) {
  ...
  for (var i = 0; i < count; i++) {
    // 尺寸: 画布12-16%(对标参考比例,比之前大)
    var size = Math.round(TPL_CW * (0.11 + rng(seed + i * 313) * 0.06));
    var cx = Math.round(TPL_CW * (0.08 + rng(seed + i * 617) * 0.72));
    var cy = Math.round(TPL_CH * (0.48 + rng(seed + i * 419) * 0.42));
    var rot = (rng(seed + i * 523) - 0.5) * 10;

    // PNG贴纸优先(80%), Canvas兜底(20%)
    var useCustom = customStickers.length > 0 && rng(seed + i * 691) < 0.80;
    ...
    try {
      ...
      ctx.beginPath(); ctx.arc(0, 0, size/2 + pad, 0, Math.PI*2); ctx.fill();
      ctx.shadowColor = 'transparent';
      ctx.beginPath(); ctx.arc(0, 0, size/2, 0, Math.PI*2); ctx.clip();

      if (useCustom) {
        var csIdx = Math.floor(rng(seed + i * 877) * customStickers.length);
        if (usedStickers['custom_' + csIdx]) { useCustom = false; }
        else {
          usedStickers['custom_' + csIdx] = true;
          var cs = customStickers[csIdx];
          var img = new Image(); img.src = cs.dataUrl;
          if (img.complete && img.naturalWidth > 0) {
            ctx.drawImage(img, -size/2, -size/2, size, size);
          } else { useCustom = false; }
        }
      }

      if (!useCustom && cuteKeys.length > 0) {
        // Canvas兜底: 去重
        var tries = 0, idx;
        do { idx = Math.floor(rng(seed + i * 7919 + tries * 137) * cuteKeys.length); tries++; }
        while (usedStickers[idx] && tries < 10);
        usedStickers[idx] = true;
        var drawFn = cuteFns[cuteKeys[idx]];
        // 贴纸函数内部坐标: (50,50)是中心 → 需translate(-size/2,-size/2)使其中心对齐(0,0)
