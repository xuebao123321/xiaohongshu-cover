...
/** 31. 全身小猫(坐姿) */
export function drawFullBodyCatSticker(ctx, x, y, size, accentColor = '#FFD9A8') {
  ctx.save(); ctx.translate(x, y); const s = size / 100; ctx.scale(s, s);
  // 身体(椭圆)
  ctx.fillStyle = accentColor; ctx.beginPath(); ctx.ellipse(50, 60, 24, 28, 0, 0, Math.PI * 2); ctx.fill();
  // 头
  ctx.beginPath(); ctx.arc(50, 28, 22, 0, Math.PI * 2); ctx.fill();
  // 耳朵
  ctx.beginPath(); ctx.moveTo(30, 16); ctx.lineTo(34, -2); ctx.lineTo(42, 18); ctx.closePath(); ctx.fill();
  ctx.beginPath(); ctx.moveTo(70, 16); ctx.lineTo(66, -2); ctx.lineTo(58, 18); ctx.closePath(); ctx.fill();
  ctx.fillStyle = '#FFB8C8';
  ctx.beginPath(); ctx.moveTo(34, 16); ctx.lineTo(36, 4); ctx.lineTo(40, 18); ctx.closePath(); ctx.fill();
  ctx.beginPath(); ctx.moveTo(66, 16); ctx.lineTo(64, 4); ctx.lineTo(60, 18); ctx.closePath(); ctx.fill();
  // 眼睛
  ctx.fillStyle = '#3A3A3A'; ctx.beginPath(); ctx.arc(40, 26, 4, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(60, 26, 4, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#FFF'; ctx.beginPath(); ctx.arc(42, 24, 1.8, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(62, 24, 1.8, 0, Math.PI * 2); ctx.fill();
  // 鼻子嘴
  ctx.fillStyle = '#FF9E9E'; ctx.beginPath(); ctx.moveTo(50, 30); ctx.lineTo(48, 33); ctx.lineTo(52, 33); ctx.closePath(); ctx.fill();
  ctx.strokeStyle = '#3A3A3A'; ctx.lineWidth = 1.2; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(44, 34); ctx.quadraticCurveTo(48, 38, 50, 34);
  ctx.quadraticCurveTo(52, 38, 56, 34); ctx.stroke();
  // 前爪
  ctx.fillStyle = accentColor; ctx.beginPath(); ctx.ellipse(36, 74, 8, 12, 0.3, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(64, 74, 8, 12, -0.3, 0, Math.PI * 2); ctx.fill();
  // 尾巴
  ctx.strokeStyle = accentColor; ctx.lineWidth = 8; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(68, 56); ctx.quadraticCurveTo(82, 40, 76, 16); ctx.stroke();
  // 胡须
  ctx.strokeStyle = 'rgba(0,0,0,0.15)'; ctx.lineWidth = 0.8;
  [18, 14].forEach(dx => { ctx.beginPath(); ctx.moveTo(28, 28); ctx.lineTo(28 - dx, 26); ctx.stroke(); });
  [18, 14].forEach(dx => { ctx.beginPath(); ctx.moveTo(72, 28); ctx.lineTo(72 + dx, 26); ctx.stroke(); });
  // 腮红
  ctx.fillStyle = 'rgba(255,160,160,0.4)'; ctx.beginPath(); ctx.ellipse(30, 34, 5, 3, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(70, 34, 5, 3, 0, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
}
...
export function drawFullBodyDogSticker(ctx, x, y, size, accentColor = '#F5D0A0') {
  ...
  // 头
  ctx.beginPath(); ctx.ellipse(50, 30, 20, 22, 0, 0, Math.PI * 2); ctx.fill();
  // 垂耳
  ctx.fillStyle = darken(accentColor, 0.2);
  ctx.beginPath(); ctx.ellipse(28, 34, 10, 16, -0.4, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(72, 34, 10, 16, 0.4, 0, Math.PI * 2); ctx.fill();
  // 眼睛
  ctx.fillStyle = '#2D2A26'; ctx.beginPath(); ctx.arc(40, 26, 3.5, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(60, 26, 3.5, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#FFF'; ctx.beginPath(); ctx.arc(41, 24, 1.5, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(61, 24, 1.5, 0, Math.PI * 2); ctx.fill();
  // 鼻子
  ctx.fillStyle = '#3A2E1A'; ctx.beginPath(); ctx.ellipse(50, 32, 5, 3.5, 0, 0, Math.PI * 2); ctx.fill();
  // 舌头
  ctx.fillStyle = '#FF8888'; ctx.beginPath(); ctx.ellipse(50, 40, 4, 6, 0, 0, Math.PI * 2); ctx.fill();
  // 腿
  ctx.fillStyle = accentColor;
  [{x:34,y:78,w:8,h:16},{x:66,y:78,w:8,h:16}].forEach(l => {
    ctx.beginPath(); ctx.ellipse(l.x, l.y, l.w, l.h, 0, 0, Math.PI * 2); ctx.fill();
  });
  // 尾巴
  ctx.strokeStyle = accentColor; ctx.lineWidth = 6; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(66, 58); ctx.quadraticCurveTo(80, 50, 74, 36); ctx.stroke();
  ctx.restore();
}

/** 33. Q版男孩角色 */
export function drawBoyCharacterSticker(ctx, x, y, size, accentColor = '#FFE0BB') {
  ctx.save(); ctx.translate(x, y); const s = size / 100; ctx.scale(s, s);
  // 身体
  ctx.fillStyle = '#5B9BD5'; ctx.beginPath(); ctx.ellipse(50, 68, 20, 22, 0, 0, Math.PI * 2); ctx.fill();
  // 头
  ctx.fillStyle = accentColor; ctx.beginPath(); ctx.arc(50, 32, 20, 0, Math.PI * 2); ctx.fill();
  // 头发(刘海)
  ctx.fillStyle = '#3A2E1A'; ctx.beginPath(); ctx.ellipse(50, 18, 18, 10, 0, Math.PI, 0); ctx.fill();
  ctx.beginPath(); ctx.moveTo(32, 24); ctx.quadraticCurveTo(36, 10, 50, 14);
  ctx.quadraticCurveTo(64, 10, 68, 24); ctx.quadraticCurveTo(64, 28, 50, 22);
  ctx.quadraticCurveTo(36, 28, 32, 24); ctx.fill();
  // 眼睛
  ctx.fillStyle = '#1A1A1A'; ctx.beginPath(); ctx.arc(40, 32, 3, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(60, 32, 3, 0, Math.PI * 2); ctx.fill();
  // 微笑
  ctx.strokeStyle = '#C08060'; ctx.lineWidth = 1.5; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.arc(50, 38, 6, 0.1, Math.PI - 0.1); ctx.stroke();
  // 腮红
  ctx.fillStyle = 'rgba(255,160,140,0.35)';
  ctx.beginPath(); ctx.ellipse(32, 36, 5, 3, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(68, 36, 5, 3, 0, 0, Math.PI * 2); ctx.fill();
  // 帽子
  ctx.fillStyle = '#E85D3A'; ctx.beginPath(); ctx.ellipse(50, 16, 10, 4, 0, 0, Math.PI * 2); ctx.fill();
...
export function drawGirlCharacterSticker(ctx, x, y, size, accentColor = '#FFE8EE') {
  ctx.save(); ctx.translate(x, y); const s = size / 100; ctx.scale(s, s);
  // 身体(裙子)
  ctx.fillStyle = '#F8A0B8'; ctx.beginPath(); ctx.moveTo(32, 64); ctx.lineTo(20, 84); ctx.lineTo(80, 84); ctx.lineTo(68, 64); ctx.closePath(); ctx.fill();
  // 头
  ctx.fillStyle = accentColor; ctx.beginPath(); ctx.arc(50, 30, 20, 0, Math.PI * 2); ctx.fill();
  // 双马尾
  ctx.fillStyle = '#6B3A2A';
  [{cx:24,cy:18},{cx:76,cy:18}].forEach(p => {
    ctx.beginPath(); ctx.arc(p.cx, p.cy, 7, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(p.cx + (p.cx < 50 ? -3 : 3), p.cy + 14, 5, 12, 0.3, 0, Math.PI * 2); ctx.fill();
  });
  // 刘海
  ctx.beginPath(); ctx.ellipse(50, 18, 14, 8, 0, Math.PI, 0); ctx.fill();
  // 眼睛(大圆)
  ctx.fillStyle = '#1A1A1A'; ctx.beginPath(); ctx.arc(38, 28, 4, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(62, 28, 4, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#FFF'; ctx.beginPath(); ctx.arc(40, 26, 3, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(64, 26, 3, 0, Math.PI * 2); ctx.fill();
  // 睫毛
  ctx.strokeStyle = '#1A1A1A'; ctx.lineWidth = 1.5; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(34, 24); ctx.quadraticCurveTo(32, 20, 36, 22); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(66, 24); ctx.quadraticCurveTo(68, 20, 64, 22); ctx.stroke();
  // 微笑
  ctx.strokeStyle = '#E8889A'; ctx.lineWidth = 1.5; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.arc(50, 36, 5, 0.1, Math.PI - 0.1); ctx.stroke();
  // 腮红
  ctx.fillStyle = 'rgba(255,160,180,0.4)';
  ctx.beginPath(); ctx.ellipse(30, 34, 5, 3, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(70, 34, 5, 3, 0, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
}
